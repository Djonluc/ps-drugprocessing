...
RegisterServerEvent('ps-drugprocessing:processPoppyResin', function()
	...
	if Player.Functions.RemoveItem('poppyresin', Config.HeroinProcessing.Poppy) then
	  if Player.Functions.RemoveItem("syringe", Config.MethProcessing.HydAcid) then
		...
		if Player.Functions.AddItem('heroin', 1) then
			...
			TriggerClientEvent("inventory:client:ItemBox", source, QBCore.Shared.Items['syringe'], "remove", Config.HeroinProcessing.Poppy)
		...
		Player.Functions.AddItem('syringe', 1)
	end	
	else
