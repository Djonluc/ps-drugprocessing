...
local function SpawnCocaPlants()
    ...
    -- Register targets for Coca Plants
    for _, coca in ipairs(CocaPlants) do
        exports['ox_target']:AddTargetEntity(coca, {
            options = {
                {
                    name = 'pickCocaLeaves',
                    icon = 'fas fa-leaf',
                    label = Lang:t("interaction.pickup_coca_leaves"),
                    action = function()
                        TriggerEvent('ps-drugprocessing:pickCocaLeaves')
                    end
                }
            },
            distance = 2.0
